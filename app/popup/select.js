...
class SelectAndUpload extends React.Component {
  ...
  componentWillMount() {
    request.get(`http://www.codesign.io/api/users/${this.state.me.id}/folders`).then((data1)=>{
      ...
      return request.get('http://www.codesign.io/api/folders/'+ activeFolder.id + '/boards?'+qs.stringify({embed: [{l:'boards_codes'}, {l: 'posts'}]})).then((data2)=>{
  ...
  setFolder(e) {
    ...
    request.get('http://www.codesign.io/api/folders/'+ activeFolder.id + '/boards?'+qs.stringify({embed: [{l:'boards_codes'}, {l: 'posts'}]})).then((data)=>{
