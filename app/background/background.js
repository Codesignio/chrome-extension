...
chrome.runtime.onInstalled.addListener(function(){
  chrome.tabs.create({'url': 'http://www.codesign.io/checkauthorization'});
});

chrome.runtime.setUninstallURL('http://www.codesign.io/chrome?uninstalled=true');
...
var messageListener = function(request, sender, callback) {
  ...
  genRunner(function*() {
    ...
    if (request.msg == 'stopOauth'){
      ...
      var user = yield httprequest.get('http://www.codesign.io/api/users/me');

      if(user.role != 'guest'){
        chrome.tabs.remove(tab.id);
        !firstAuthorization && chrome.tabs.create({'url': 'http://www.codesign.io/chrome?successfully_installed=true'});
        ...
        if (!request.fromSite) chrome.tabs.create({'url': 'http://www.codesign.io/syncauthorization', selected: false});
      } else {
        localStorage.token = '';
        firstAuthorization = true;
        localStorage.firstAuthorization = 'true';
        chrome.tabs.update(tab.id, {url: 'http://www.codesign.io/chrome?extension_authorization=true'})
      }
      startOauth = null;
    }
...
var uploadImages = function*(req){
  ...
  if(activeBoard.id == 'new_board'){
    activeBoard = yield httprequest.post('http://www.codesign.io/api/folders/'+ activeFolder.id + '/boards', {}, {title: capturedImages[0].pageTitle, status: "AC", post_preview_id: ''});
    readCodeObj = yield httprequest.post('http://www.codesign.io/api/boards/'+ activeBoard.id + '/boards_codes', {}, {code: randomStr(6), role: 'CL', boards_id: activeBoard.id});
    activeBoard.boards_codes = [readCodeObj];
    createBoard = true;
    yield httprequest.post('http://www.codesign.io/api/boards/'+ activeBoard.id + '/boards_codes', {}, {code: randomStr(6), role: 'CB', boards_id: activeBoard.id});
  }
  ...
  for (var i in capturedImages){
    ...
    var post = yield httprequest.post('http://www.codesign.io/api/boards/'+ activeBoard.id + '/posts', {}, {title: capturedImage.url + " " + (new Date).toString(), position: position});
    ...
    yield httprequest.put('http://www.codesign.io/api/boards/'+ activeBoard.id, {}, {post_preview_id: post.id});
    ...
    var image = yield httprequest.post('http://www.codesign.io/api/posts/'+post.id+'/images', {}, file);
    yield httprequest.put('http://www.codesign.io/api/posts/'+ post.id, {}, {imageversion_id: image.id});
    ...
    if(capturedImage.pins){
      ...
      for (var i = 0; i < capturedImage.pins.length;i++) {
        ...
        var task = yield httprequest.post('http://www.codesign.io/api/posts/' + post.id + '/tasks', {}, {
          title: pin.text,
        ...
        if (pin.children.length) {
          for (var i = 0; i < pin.children.length; i++) {
            ...
            yield httprequest.post('http://www.codesign.io/api/tasks/' + task.id + '/comments', {}, {
              title: comment.text
            })
          }
  ...
  window.open("http://www.codesign.io/board/" + (readCodeObj && readCodeObj.code || activeBoard.boards_codes.filter((bc=>bc.role=="CL"))[0].code));
  chrome.browserAction.setBadgeText({text: ''});
